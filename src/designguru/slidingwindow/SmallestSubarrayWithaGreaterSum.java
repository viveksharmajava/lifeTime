...
public static void smallestSubarrayWithgreaterSum(int [] arr , long target) {
		...
		for(int i = 0 ; i < arr.length ; i++) {
			...
			if( sum >= target) {
				...
				while((sum-arr[start]) >= target && start < i){
					
					sum -= arr[start];
					System.out.println("Sum ="+sum+"\t len "+(i-start+1));
					start++;
				}
				if(min_len > (i-start+1) || prev_max > sum){
					min_len = i-start+1;
					prev_max = sum;
					s = start;
		...
		if(min_len  < Integer.MAX_VALUE) {
			System.out.println("Subaray  start "+s+"end "+(s+min_len)+" elements"+Arrays.toString(Arrays.copyOfRange(arr, s, s+min_len)));
		}else {
